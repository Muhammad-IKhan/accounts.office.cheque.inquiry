update and correct it by yourself dont change the other code only change the empty data to - for Nan for amount and every column ```// Global variables to store DOM elements and data
...
function createTableRow(element) {
    ...
    Object.keys(columns).forEach(field => {
        ...
        if (field === 'AMOUNT') {
            ...
            try {
                // If the value is empty or NaN, replace it with '-'
                if (value === '-' || isNaN(parseFloat(value))) {
                    value = '-';
                } else {
                    // Format the amount value if it's valid
                    value = parseFloat(value).toLocaleString('en-US');
                }
            } catch (error) {
                console.warn(`Invalid amount value detected: ${value}, defaulting to "-"`);
                value = '-';
            }
        }
